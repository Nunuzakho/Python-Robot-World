...
def get_obstacles():
    '''This function gets randomly printed obstacles from the default obstacle list.
    * min_y, max_y and min_x, max_x calculates the limited areas.
    '''

    rand_amount = random.randint(1, 10)
    obstacles =[(random.randint(min_x, max_y),random.randint(min_y, max_y))
        for num in range(rand_amount)]
    white_block = []
    for x in range(min_x, max_x):
        for y in range(min_y, max_y):
            for ob in obstacles:
                if x in range(ob[0], ob[0]+5) and y in range(ob[1], ob[0]+5):
                    continue
                else:
                    white_block.append((x,y))

    return obstacles, white_block
...
def is_position_blocked(x,y,obstacles):
    ...
    * The red blocks(set as blocked co-ordinates) represents the blocked path that the Robot won't be able to access.
    ...
    for obstacle in obstacles:
        if x in range(obstacle[0], obstacle[0]+obstacle_size) and y in range(obstacle[1], obstacle[1]+obstacle_size):
            return True
    return False
...
def is_path_blocked(x1,y1,x2,y2,obstacles):
    ...
    for num in range(min(y1, y2), max(y1, y2)+1):
        if is_position_blocked(num, x1,obstacles):
            return True
    return False
